...
{
    public partial class MedicineFieldForm : Form
    {
        ...
        public MedicineFieldForm(PIPDataSet.MedicineFieldRow data_row, PLConstants.enum_data_operation operation, DataTable selected_degrees)//checked
        {
            InitializeComponent();
            ...
            this.selected_fields = selected_degrees;
            ...
            {
                this.Text = "ایجاد رشته پزشکی جدید";
            ...
            else if (operation == PLConstants.enum_data_operation.edit_data)
            {
                ...
                int tc = 0;
                int cc = 0;
                this.Text = "ویرایش اطلاعات رشته پزشکی";
                for (int i = 0; i < properties_count; i++)
                {
                    if (allow_validating[i])
                    {
            ...
            else if (operation == PLConstants.enum_data_operation.batch_edit)
            {
                errors = new bool[properties_count];
                error_count = 0;
                this.Text = "ویرایش دسته ای اطلاعات چند رشته پزشکی";
        ...
        private void save_button_Click(object sender, EventArgs e)//checked
        {
            ...
            else if (form_operation == PLConstants.enum_data_operation.new_data)
            {
                ...
                switch (pgs_dialog.user_choice)
                {
                    case PLConstants.enum_dialog_options.yes:
                        string add_result = "";
                        mf_adapter.AddField((string)new_data[1],(string)new_data[2], ref add_result);
            ...
            else if (form_operation == PLConstants.enum_data_operation.edit_data)
            {
                ...
                switch (pgs_dialog.user_choice)
                {
                    case PLConstants.enum_dialog_options.yes:
                        string edit_result = "";
                        mf_adapter.EditFields(false, (int)new_data[0], PLGlobalFuncs.emptyIntIDsDataTable(), (string)new_data[1], (string)new_data[2],
                                              ref edit_result);
            ...
            else if (form_operation == PLConstants.enum_data_operation.batch_edit)
            {
                List<string> message_text = new List<string>();
                message_text.Add(PLConstants.question_save_edit);
                message_text.Add("ویرایش دسته ای اطلاعات " + selected_fields.Rows.Count + " مورد");
                message_text.Add("اطلاعات جدید");
                for (int i = 0; i < properties_count; i++)
                {
                    if (allow_batch_validating[i])
                ...
                switch (pgs_dialog.user_choice)
                {
                    case PLConstants.enum_dialog_options.yes:
                        string edit_result = "";
                        mf_adapter.EditFields(true, null, selected_fields, (string)new_data[1], (string)new_data[2],
                                              ref edit_result);
                        result_enum = PLEnumFuncs.opResultToEnum(edit_result);
                        pgs_dialog.operationResult(result_enum, PLConstants.enum_information_part.other);
                        switch (result_enum)
